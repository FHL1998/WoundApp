...
def dice_coef(y_true, y_pred):
    smooth = 0.00001  # 防止分母为0
    y_true_f = K.flatten(y_true)  # 将y_true拉为一维
    y_pred = K.cast(y_pred, 'float32')  # 张量数据类型转换
    # if y_pred > 0.5 , means it contain some valid # obj(may be 1,2.. y_pred)
    # needed to get a mask y_pred_f by thresholding y_pred, then multiply with y_true_f
